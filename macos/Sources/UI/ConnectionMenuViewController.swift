...
/// A single connection choice produced by `ConnectionMenuViewController`.
///
/// Field priority when fed into `ZonvieCore.start()` is
/// `connectionConfig > CLI flags > config.toml` (see `ZonvieCore.start`).
/// The three connection modes are mutually exclusive and selected by the
/// dialog's active tab: SSH is active iff `sshHost` is non-empty, devcontainer
/// iff `devcontainerWorkspace` is non-empty, otherwise the connection is local.
struct ConnectionConfig {
    var name: String = ""
    var nvimPath: String = ""      // empty = use default from config/CLI

    var sshHost: String = ""       // empty = not SSH
    var sshPort: String = ""       // empty = default (22)
...
/// Sheet presented at startup when launched with `--dialog`.
...
final class ConnectionMenuViewController: NSViewController {
    ...
    /// Initial tab to focus ("local" / "ssh" / "devcontainer"). Set by the
    /// caller from the CLI so `--ssh --dialog` / `--devcontainer --dialog` open
    /// on the matching tab.
    var initialTab: String = "local"
    /// Field values to pre-fill (from the CLI). Empty fields stay empty.
    var seedConfig = ConnectionConfig()
    ...
    override func loadView() {
        ...
        // Pre-fill fields and focus the tab requested by the CLI (`--ssh` /
        // `--devcontainer` accompanying `--dialog`); Local by default.
        if !seedConfig.nvimPath.isEmpty { nvimPathField.stringValue = seedConfig.nvimPath }
        sshHostField.stringValue = seedConfig.sshHost
        sshPortField.stringValue = seedConfig.sshPort
        sshIdentityField.stringValue = seedConfig.sshIdentity
        devcontainerWorkspaceField.stringValue = seedConfig.devcontainerWorkspace
        devcontainerConfigField.stringValue = seedConfig.devcontainerConfig
        devcontainerRebuildCheck.state = seedConfig.devcontainerRebuild ? .on : .off
        tabView.selectTabViewItem(withIdentifier: initialTab)
    }
    ...
    private func layoutDevcontainerTab(in parent: NSView, width: CGFloat) {
        ...
        // Leave room on the right for a Browse button (directory picker).
        let browseW: CGFloat = 74
        let wsFieldW = max(60, width - 94 - browseW - 8)
        devcontainerWorkspaceField.placeholderString = "/path/to/project"
        devcontainerWorkspaceField.font = NSFont.systemFont(ofSize: 13)
        devcontainerWorkspaceField.frame = NSRect(x: x + 84, y: y - fieldHeight + 2, width: wsFieldW, height: fieldHeight)
        parent.addSubview(devcontainerWorkspaceField)

        let browseBtn = NSButton(title: "Browse…", target: self, action: #selector(browseDevcontainerWorkspace))
        browseBtn.bezelStyle = .rounded
        browseBtn.font = NSFont.systemFont(ofSize: 12)
        browseBtn.frame = NSRect(x: x + 84 + wsFieldW + 8, y: y - fieldHeight, width: browseW, height: fieldHeight + 2)
        parent.addSubview(browseBtn)
        y -= 32
    ...
    /// Directory picker for the devcontainer workspace folder.
    @objc private func browseDevcontainerWorkspace() {
        let panel = NSOpenPanel()
        panel.canChooseDirectories = true
        panel.canChooseFiles = false
        panel.allowsMultipleSelection = false
        panel.prompt = "Select"
        panel.message = "Choose the devcontainer workspace folder"
        let current = devcontainerWorkspaceField.stringValue
        if !current.isEmpty {
            panel.directoryURL = URL(fileURLWithPath: (current as NSString).expandingTildeInPath)
        }
        // App-modal so it works while this view is presented as a sheet.
        if panel.runModal() == .OK, let url = panel.url {
            devcontainerWorkspaceField.stringValue = url.path
        }
    }
    ...
    @objc private func connectAction() {
        ...
        case "devcontainer":
            ...
            config.devcontainerRebuild = devcontainerRebuildCheck.state == .on
        default:
